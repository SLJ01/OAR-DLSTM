...
def data_Preparation_submodel3_DS1():
    ...
    temp_33 = slj_arr[0:1, 3:4][0][0]  # 热风阀
    temp_167 = slj_arr[0:1, 10:11][0][0]  # 燃气阀
    temp_95 = slj_arr[0:1, 6:7][0][0]  # 喷水阀
    temp_24 = slj_arr[0:1, 11:12][0][0]  # 鼓风阀开度
    temp_22 = slj_arr[0:1, 12:13][0][0]  # 补风阀开度
    id_init = []
    ...
    for n in range(len(id_init)-4):
        ...
        last_33 = data1_arr[119:120, 12:13][0][0]
        last_167 = data1_arr[119:120, 27:28][0][0]
        last_95 = data1_arr[119:120, 19:20][0][0]
        last_24 = data1_arr[119:120, 8:9][0][0]
        last_22 = data1_arr[119:120, 6:7][0][0]
        ...
        if ((temp_33 == last_33) and (temp_167 == last_167) and (temp_95 == last_95) and (temp_24 == last_24) and (
            ...
            for x in range(move_num):
                ...
                b = data1_values[x+look_back+0:x+look_back+predict_size+0, 0]#中间空0个点
...
def train_sub_model3_DS1():
    ...
    history = model.fit(trainX, trainY, batch_size=80,epochs=1050, verbose=2, validation_split=0.03)#batch_size=32
    model.save('model3_DS1.h5')
    ...
    model = load_model('model3_DS1.h5')
...
def train_sub_model3_DS2():
    look_back = 40
    predict_size = 30
    trainX, trainY = data_Preparation_submodel3_DS2()
    trainX = np.array(trainX)
    trainY = np.array(trainY)
    y_max = np.max(trainY)
    y_min = np.min(trainY)
    ...
    testX, testY, d_test, y_min_test = data_test_DS2()
    testX = np.reshape(testX, (testX.shape[0], look_back, testX.shape[2]))
    ...
    model = baseline_model()
    history = model.fit(trainX, trainY, batch_size=80,epochs=750, verbose=2, validation_split=0.03)#batch_size=32
    model.save('model3_DS2.h5')
    ...
    model = load_model('model3_DS2.h5')
    trainPredict = model.predict(trainX)
    testPredict = model.predict(testX)
    trainY1 = trainY_normal.reshape(-1, 1)
    testY1 = testY.reshape(-1, 1)
...
if __name__ == '__main__':
    train_sub_model3_DS1()
    train_sub_model3_DS2()
