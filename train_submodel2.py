...
# trian_submodel2 on the DS1
def data_Preparation_submodel2_DS1():
    look_back = 40
    predict_size = 30
    move_num =5
    idsum = []
    ...
    for x in range(len(id_init)):
        sql2='select ID,TIMESTAMP,L0024,L0167 from DB_JIANGYIN  where id between %s and %s' % (
            id_init[x] -20 , id_init[x])
        data0 = pd.read_sql(sql2, conn)
        data0_arr = np.array(data0)
        init_24=data0_arr[0,2]
        init_167=data0_arr[0,3]
        last_24=data0_arr[20,2]
        last_167=data0_arr[20,3]
        last2_24=data0_arr[19,2]
        last2_167=data0_arr[19,3]
        if((last_167!=last2_167) and (last_24==init_24)):
            id_init2.append(id_init[x])
    id_last = []
    for n in range(len(id_init2) - 2):
        sql3 = 'select ID,TIMESTAMP,L0002,L0005,L0008,L0069,L0022,L0023,L0024,L0026,L0027,L0028,L0033,L0034,L0040,L0042,L0070,L0093,L0094,L0095,L0096,L0097,L0157,L0158,L0159,L0160,L0162,L0167,L0013 from DB_JIANGYIN  where id between %s and %s' % (
            id_init2[n] - 39, id_init2[n] + 100)  # delat time=8，
        ...
        last_33 = data1_arr[139:140, 12:13][0][0]
        last_167 = data1_arr[139:140, 27:28][0][0]  #
        last_95 = data1_arr[139:140, 19:20][0][0]  #
        last_24 = data1_arr[139:140, 8:9][0][0]  #
        last_22 = data1_arr[139:140, 6:7][0][0]  #
        if ((temp_33 == last_33) and (temp_167 == last_167) and (temp_95 == last_95) and (temp_24 == last_24) and (
            ...
            for x in range(move_num):
                ...
                b = data1_values[x + look_back + 20:x + look_back + predict_size + 20, 0]
...
def train_sub_model2_DS1():
    look_back = 40
    predict_size = 30
    trainX, trainY = data_Preparation_submodel2_DS1()
    ...
    history = model.fit(trainX, trainY, batch_size=80,epochs=1600, verbose=2, validation_split=0.03)#batch_size=32
    model.save('model2_DS1.h5')
...
def data_Preparation_submodel2_DS2():
    look_back = 40
    predict_size = 30
    move_num = 5
    idsum = []
...
def train_sub_model2_DS2():
    look_back = 40
    predict_size = 30
    trainX, trainY = data_Preparation_submodel2_DS2()
    trainX = np.array(trainX)
    trainY = np.array(trainY)
    y_max = np.max(trainY)
    y_min = np.min(trainY)
    ...
    model = baseline_model()
    history = model.fit(trainX, trainY, batch_size=80,epochs=1500, verbose=2, validation_split=0.03)#batch_size=32
    model.save('model2_DS2.h5')
...
if __name__ == '__main__':
    train_sub_model2_DS1()
    train_sub_model2_DS2()
